...
struct ResAnmClr {
    ...
    inline ResAnmClrData& ref() const { return mAnmClr.ref(); }

    inline bool CheckRevision() const { return ref().mRevision == REVISION; }

    AnmPolicy GetAnmPolicy() const { return ref().mAnmPolicy; }

    int GetNumFrame() const { return ref().mNumFrames; }

    inline const ResAnmClrMatData* GetMatAnm(
        u32 i
    ) const {
        return static_cast<const ResAnmClrMatData*>(
            mAnmClr.ofs_to_obj<ResDic>(ref().mMatDictOffset)[i]
        );
    }
