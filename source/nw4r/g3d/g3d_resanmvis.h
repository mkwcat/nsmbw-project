...
struct ResAnmVis {
    ...
    inline ResAnmVisData& ref() const { return mAnmVis.ref(); }

    inline bool CheckRevision() const { return ref().mRevision == REVISION; }

    inline const ResAnmVisNodeData* GetNodeAnm(
        u32 i
    ) const {
        return static_cast<const ResAnmVisNodeData*>(
            mAnmVis.ofs_to_obj<ResDic>(ref().mNodeDictOffset)[i]
        );
    }

    AnmPolicy GetAnmPolicy() const { return ref().mInfo.mAnmPolicy; }

    int GetNumFrame() const { return ref().mInfo.mNumFrames; }

    int GetNumNode() const { return ref().mInfo.mNumNodes; }
