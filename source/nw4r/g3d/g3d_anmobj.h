...
inline PlayPolicyFunc GetAnmPlayPolicy(
    AnmPolicy policy
) {
...
struct FrameCtrl {
    FrameCtrl(
        f32 f_1, f32 f_2, PlayPolicyFunc policy
    )
        : mFrame(0.0f)
        , mRate(1.0f)
        , FLOAT_0x8(f_1)
    ...
    f32 GetFrm() const { return mFrame; }
    ...
    void UpdateFrm() { SetFrm(mFrame + mRate * smBaseUpdateRate); }
