...
#define NW4R_G3D_TYPE_OBJ_DECL(VAL) \
    static const nw4r::g3d::G3dObj::ResNameDataT<sizeof(#VAL)> TYPE_NAME
...
namespace g3d {
namespace detail {
inline void* AllocFromAllocator(
    MEMAllocator* pAllocator, u32 size
) {
    return MEMAllocFromAllocator(pAllocator, size);
}

inline void FreeToAllocator(
...
public:
    ...
    struct TypeObj {
        ...
        template <u32 N>
        TypeObj(
            const ResNameDataT<N>& pRes
        )
            : mData(reinterpret_cast<const TypeObjData*>(&pRes)) {}

        u32 GetTypeID() const { return reinterpret_cast<u32>(mData); }

        const char* GetTypeName() const { return mData->mName; }
...
public:
    ...
    G3dObj(
        MEMAllocator* pAllocator, G3dObj* pParent
    )
        : mParent(pParent)
