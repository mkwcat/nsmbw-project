...
#ifndef NO_STRTOLD
extern long double __strtold(
    int max_width, int (*ReadProc)(void*, int, int), void* ReadProcArg, int* chars_scanned,
    int* overflow
);
#endif
...
extern unsigned long long __strtoull(
    int base, int max_width, int (*ReadProc)(void*, int, int), void* ReadProcArg,
    int* chars_scanned, int* negative, int* overflow
);
#endif

extern unsigned long __strtoul(
    int base, int max_width, int (*ReadProc)(void*, int, int), void* ReadProcArg,
    int* chars_scanned, int* negative, int* overflow
);
...
static const char* parse_format(
    const char* format_string, scan_format* format
) {
    ...
    case 'G':
        if (f.argument_options == char_argument || f.argument_options == short_argument ||
            f.argument_options == intmax_argument || f.argument_options == size_t_argument ||
...
static int __sformatter(
    int (*ReadProc)(void*, int, int), void* ReadProcArg, const char* format_str, va_list arg,
    int is_secure
) {
    ...
    while (!terminate && (format_char = *format_ptr) != 0) {
        ...
        if ((format.conversion_char != 'n') && (!match_failure) &&
            (*ReadProc)(ReadProcArg, 0, __TestForError)) {
            if (!is_secure) {
                goto exit;
            } else {
                match_failure = 1;
...
#ifndef NO_STRTOULL
                    u_long_long_num = __strtoull(
                        base, format.field_width, ReadProc, ReadProcArg, &num_chars, &negative,
                        &overflow
                    );
#else
                    assert(!"Long long format argument with strtoull disabled");
#endif
                } else {
                    u_long_num = __strtoul(
                        base, format.field_width, ReadProc, ReadProcArg, &num_chars, &negative,
                        &overflow
                    );
                }
        ...
        assign_float:
            ...
            break;
#else
            assert(!"Floating pointer format argument with strtold disabled");
#endif
            ...
            if (arg_ptr) {
                ...
                if (match_failure) {
                    ...
                    while (format.field_width-- &&
                           ...
                           ((rval = ((*ReadProc)(ReadProcArg, 0, __GetAChar))) != EOF)) {
                        c = rval;
...
inline int vsscanf(
    const char* s, const char* format, va_list arg
) {
    __InStrCtrl isc;
    isc.NextChar = (char*) s;
...
int sscanf(
    const char* s, const char* pFormat, ...
) {
    va_list args;
