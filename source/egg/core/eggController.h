...
public:
    ...
    f32 getFSStickX() const { return ex_status.fs.stick.x; }

    f32 getFSStickY() const { return ex_status.fs.stick.y; }
    ...
    eCoreDevType getDevType() const { return static_cast<eCoreDevType>(dev_type); }

    bool isCore() const { return getDevType() == cDEV_CORE; }

    bool isFreestyle() const { return getDevType() == cDEV_FREESTYLE; }

    bool isClassic() const { return getDevType() == cDEV_CLASSIC; }

    u32 getHold() const { return hold; }

    s8 getDpdValidFlag() const { return dpd_valid_fg; };

    Vector2f getAccelVertical() { return Vector2f(acc_vertical.x, acc_vertical.y); }

    const Vec& getAccel() const { return acc; }
};
...
public:
    ...
    /* VT+0x08 */
    virtual void setPosParam(
        f32 v1, f32 v2
    ) {}

    /* VT+0x0C */
    virtual void setHoriParam(
        f32 v1, f32 v2
    ) {}

    /* VT+0x10 */
    virtual void setDistParam(
        f32 v1, f32 v2
    ) {}

    /* VT+0x14 */
    virtual void setAccParam(
        f32 v1, f32 v2
    ) {}
...
public:
    inline constexpr auto getCoreController(
        this auto& self
    ...
    inline constexpr auto getGCController(
        this auto& self
    ...
    inline constexpr auto getClassicController(
        this auto& self
...
public:
    ...
    const CoreController* getCoreController() const { return this; }

    inline bool connected() const { return mFlag.on(1); }
...
public:
    ...
    inline CoreController* getCoreController() const { return mpCore; }

    inline bool connected() const { return mpCore->connected() && mpCore->mStatus->isClassic(); }
...
public:
    ...
    inline ClassicController* getNthClassic(
        int n
    ) {
