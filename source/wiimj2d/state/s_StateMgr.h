...
public:
    sStateMgr_c(
        T& owner, const sStateIDIf_c& initialState
    )
        : mFactory(owner)
     ...
     */
    virtual void initializeState() override { mMethod.initializeStateMethod(); }

    /**
     * VT+0x10
     * Executes the current state.
     */
    virtual void executeState() override { mMethod.executeStateMethod(); }

    /**
     * VT+0x14
     * Prepares the current state for termination.
     */
    virtual void finalizeState() override { mMethod.finalizeStateMethod(); }
     ...
     */
    virtual void refreshState() override { mMethod.refreshStateMethod(); }

    /**
     * VT+0x20
     * Returns the state holder.
     */
    virtual sStateIf_c* getState() const override { return mMethod.getState(); }

    /**
     * VT+0x24
     * Returns the next state ID.
     */
    virtual const sStateIDIf_c* getNewStateID() const override { return mMethod.getNewStateID(); }

    /**
     * VT+0x28
     * Returns the current state ID.
     */
    virtual const sStateIDIf_c* getStateID() const override { return mMethod.getStateID(); }

    /**
     * VT+0x2C
     * Returns the previous state ID.
     */
    virtual const sStateIDIf_c* getOldStateID() const override { return mMethod.getOldStateID(); }
