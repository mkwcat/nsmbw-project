...
[[nsmbw(0x800E0470)]]
dMj2dGame_c* dSaveMng_c::getSaveGame(
    s8 slot
) {
    if (slot == -1) {
...
[[nsmbw(0x800E04A0)]]
dMj2dGame_c* dSaveMng_c::getTempGame(
    s8 slot
) {
    if (slot == -1) {
        slot = mData.mHeader.mLastSelectedFile;
...
[[nsmbw(0x800E1A30)]]
void dSaveMng_c::loadQuickSaveData(
    u8 file
) {
...
void dSaveMng_c::prepareSave() {
    ...
    for (int w = 0; w < WORLD_COUNT; w++) {
        for (int e = 0; e < AMBUSH_ENEMY_COUNT; e++) {
            const dInfo_c::enemy_s& enemy = info->GetWorldMapEnemy(w, e);
