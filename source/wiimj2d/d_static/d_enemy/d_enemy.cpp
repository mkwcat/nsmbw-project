...
/* 800952CC 806DA968 */  lwz      r3, m_instance__8dQuake_c@sda21;
...
u8 dEn_c::setCollTimer(
    dActor_c* player, u8 timer
) {
    return setCollTimer(player->getPlrNo(), timer);
}

u8 dEn_c::getCollTimer(
    int player
) const {
    if (player < 0 || player >= PLAYER_COUNT) {
        return 0xFF;
...
u8 dEn_c::incrementCollTimer(
    int player, int increment
) {
    if (player == -1) {
