...
static void ClampStick(
    s8* px, s8* py, s8 max, s8 xy, s8 min
) {
    int x = *px;
    int y = *py;
    int signX;
    ...
    if (xy * y <= xy * x) {
        d = xy * x + (max - xy) * y;
        if (xy * max < d) {
            x = static_cast<s8>(xy * max * x / d);
            y = static_cast<s8>(xy * max * y / d);
        }
    } else {
        d = xy * y + (max - xy) * x;
        if (xy * max < d) {
...
static void ClampTrigger(
    u8* trigger, u8 min, u8 max
) {
    if (*trigger <= min) {
        *trigger = 0;
    } else {
