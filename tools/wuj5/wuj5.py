...
def decode_u8_node(out_path, node, retained, renamed, rootname):
    name = node['name']
    if name in renamed:
        name = renamed[name]
    ...
    if node['is_dir']:
        ...
        for child in node['children']:
            decode_u8_node(out_path, child, retained, renamed, rootname)
    else:
...
def decode_u8(in_path, out_path, retained, renamed, rootname):
...
def decode(in_path, out_path, retained, renamed, rootname):
...
def encode_u8(in_path, out_path, retained, renamed, rootname):
    ext = in_path.split(os.extsep)[-2]
    root = encode_u8_node(in_path, retained, renamed)
    out_data = pack_u8(root, rootname)
    if ext == 'szs':
...
def encode(in_path, out_path, retained, renamed, rootname):
    if in_path.endswith('.arc.d') or in_path.endswith('.szs.d') or in_path.endswith('.arc.lzma.d'):
        encode_u8(in_path, out_path, retained, renamed, rootname)
        return
    ext = in_path.split(os.extsep)[-2]
    pack = ext_pack.get(ext)
    if pack is None:
...
for in_path, out_path in zip(args.inputs, args.outputs):
    operations[args.operation](in_path, out_path, args.retained, renamed, args.root)
